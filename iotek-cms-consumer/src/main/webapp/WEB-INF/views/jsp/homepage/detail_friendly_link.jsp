...
<script type="text/javascript">
	$(function() {
		...
		var linkType = '${friendlyLink.type}';
		if(linkType == 1){
			$("#pubfoe1").show();
			$("#pubfoe2").hide();
			$("#link_picture_src").hide();
		}else{
			$("#pubfoe2").show();
			$("#pubfoe1").hide();
		...
		<div class="container">
			...
			<div class="main_content">
				...
				<div class="dash_wrapper">
					<div class="radil_01">
						<c:forEach items="${friendlyLinkTypeEnum }" var="friendlyLinkType"
							...
							<span><input id="topic_type${status.index+1 }"
								type="radio"
								<c:if test="${friendlyLink.type eq friendlyLinkType.value}">checked="checked"</c:if>
								value="${friendlyLinkType.value }" name="1">
								${friendlyLinkType.label }</span>
						</c:forEach>
					</div>
					<form action="${ctx}/friendlyLinkManage/updateLink" method="post" id="word_form">
						<input type="hidden" name="type" value="1"/>
						<input type="hidden" id="linkId" name="id" value="${friendlyLink.id }"/>
						...
						<tbody>
							<tr height="50px">
								...
								<td class="seo1"><label><input class="seo_date_03"
										name="name" type="text" id="word_name" value="${friendlyLink.name }"></label></td>
							</tr>
							<tr height="50px">
								<td width="10%" class="seo"><span>*</span>链接地址：</td>
								<td class="seo1"><label><input class="seo_date_03"
										name="url" type="text" id="word_url" value="${friendlyLink.url }"></label></td>
							</tr>
							<tr height="50px">
								...
								<td class="seo1"><label><select class="seo_date_02" name="openWay" id="word_open_way" ><c:forEach items="${friendlyLinkOpenWayEnum }" var="openWay">
									<option value="${openWay.value}" <c:if test="${friendlyLink.openWay eq openWay.value }">selected="selected"</c:if>>${openWay.label}</option>
					...
					<form action="${ctx}/friendlyLinkManage/updateLink" method="post" id="picture_form">
					...
					<table width="100%" class="table_tc" id="pubfoe2"
						...
						<tbody>
							<tr height="50px">
								...
								<td class="seo1"><label><input class="seo_date_03"
										name="name" type="text" id="picture_name" value="${friendlyLink.name }"></label></td>
							</tr>
							<tr height="50px">
								<td width="12%" class="seo"><span>*</span>链接地址：</td>
								<td class="seo1"><label><input class="seo_date_03"
										name="url" type="text" id="picture_url" value="${friendlyLink.url }"></label></td>
							</tr>
							<tr height="50px">
							...
							<tr height="50px">
								<td width="12%" class="seo"><span>*</span>鼠标移动显示：</td>
								<td class="seo1"><label><input class="seo_date_03"
										name="title" type="text" id="picture_title"  value="${friendlyLink.title }"></label></td>
							</tr>
							<tr height="50px">
								<td width="12%" class="seo"><span>*</span>链接图片：</td>
								<td class="seo1"><label>
								<img class="w_240" width="240px" height="280px" id="link_picture_src" src="${img_suffix}${totalPath}"/><br>
								<input name="" class="masee" type="button" value="上传图片" id="link_picture">
								<input type="hidden"  name="filePath" value="${friendlyLink.filePath }" id="inputPicture"/>
								</label></td>
							</tr>
							<tr height="50px">
